...
namespace BLMS
{
    public partial class AuthorForm : MetroFramework.Forms.MetroForm
    {
        ...
        {
            metroGridAuthor.DataSource = data("");
        }

        private void btnAdd_Click(object sender, EventArgs e)
        {
            store(authorName.Text);
        }

        private void btnEdit_Click(object sender, EventArgs e)
        {
            update(author_id, authorName.Text);
        }
        ...
        {
            metroGridAuthor.DataSource = data(authorSearch.Text);
        }

        private void metroGridAuthor_RowHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            edit(metroGridAuthor.CurrentRow.Cells[1].Value.ToString());
        ...
        {
            using (command = new MySqlCommand("SELECT * FROM `authors` WHERE `name`='" + text + "'", con))
            {
                con.Open();
                dataReader = command.ExecuteReader();
                if (dataReader.HasRows)
                {
                    if (dataReader.Read())
                    {
                        author_id = dataReader.GetInt32("id");
                        authorName.Text = dataReader.GetString("name");
                    }
                }
                con.Close();
            }
        ...
        // Update to DB
        public void update(int author_id, string name)
        {
            if (author_id != 0)
            {
